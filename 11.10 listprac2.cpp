...
{
	char name[20];
	char id[20];	//char *id; 로 하면 엄청 복잡해지는듯..id에도 할당해줘야하는듯.. 
	struct subject *next;
};
...
int main(void)
{
	...
	struct subject *context3=(struct subject*) malloc(sizeof(struct subject*));
  	...
  	scanf("%s", &context3->id);
	//strcpy(context3->name, "English");
	//strcpy(context3->id, "789");
	    
  /*struct subject *context3=AddSubject();
	(struct subject*) malloc(sizeof(struct subject*));
  	scanf("%s", &context3->name);
	scanf("%s", &context3->id);*/
